...
struct
  ...
  struct
    ...
    fun bind (ctx:ctx) l et =
      {locals = LocalMap.insert(#locals ctx, l, et),
       ...
       imms = #imms ctx,
       pures = #pures ctx}
    fun lookup (ctx:ctx) l = valOf(LocalMap.find(#locals ctx, l))
  ...
  fun tp_extend_fresh tp =
     case tp of
        Int	=> Atom
      | Bool => Atom
      | String => Atom
      | Char => Atom
      | Pointer(t) => Pt(freshtag (), tp_extend_fresh t)
      | Array(t) => Arr(freshtag (), tp_extend_fresh t)
      | StructName(i) => St(i)
      | TypeName(i) => raise Fail "all type names should be expanded"
      | Void => Atom
      | Any => Atom
      
  fun tp_extend_mutable tp =
     case tp of
        Int	=> Atom
      | Bool => Atom
      | String => Atom
      | Char => Atom
      | Pointer(t) => Pt(Mut, tp_extend_mutable t)
      | Array(t) => Arr(Mut, tp_extend_mutable t)
      | StructName(i) => St(i)
      | TypeName(i) => raise Fail "all type names should be expanded"
      | Void => Atom
      | Any => Atom
      
  fun tp_extend_immutable tp =
    case tp of 
        Int	=> Atom
      | Bool => Atom
      | String => Atom
      | Char => Atom
      | Pointer(t) => Pt(Imm, tp_extend_immutable t)
      | Array(t) => Arr(Imm, tp_extend_immutable t)
      | StructName(i) => St(i)
      | TypeName(i) => raise Fail "all type names should be expanded"
      | Void => Atom
      | Any => Atom
  ...
  fun mark_imms' ctx v imms tp =
     case tp of
      ...
      | Void => imms
      | Any => imms
  and mark_imms_struct' ctx v imms s =
     case SymSet.member(v, s) of
        true => imms
      | false => 
         let val imms' = foldl (fn ((i,t),c) => 
                                  let val (ctx, et) = tp_extend_imm_rec c t
                                  in C.bind_field ctx s i et end)
                         ctx' (struct_fields s)
         in imms' end
  in
  fun mark_imms ctx imms tp = mark_imms' ctx (SymSet.empty) imms tp
  end
   
  fun concall ctx imms f args =
    case C.isPure f of
       true => imms (* no constraints in this case *)
     | false => foldr (fn (a, imms) => mark_imms ctx imms (syn_extended ctx a)) imms args
   
  fun constraintsE ctx imms e =
    case e of
       Local l => imms
     | Op (oper, args) => foldr (fn (e, i) => constraintsE ctx i e) imms args
     | Call (f, args) => 
          let val imms' = foldr (fn (e, i) => constraintsE ctx i e) imms args
          in concall ctx imms' f args end
     | IntConst _ => imms
     | BoolConst _ => imms
     | StringConst _ => imms
     | CharConst _ => imms
     ...
     | Null => imms
     | Result => imms
     | Length e' => constraintsE ctx imms e'
     | Old e' => constraintsE ctx imms e'
     | AllocArray (tp, e') => constraintsE ctx imms e'
     | Select (e', field) => constraintsE ctx imms e'
     | MarkedE (m) => constraintsE ctx imms (Mark.data m)
     *)
    ...
    case C.isPure ctx f of
     ...
     | false => 
         let val immutables = (map (fn a => checkMutability ctx imms (syn_extended ctx a)) args)
             val _ = TextIO.print ("call to " ^ Symbol.name f ^ ": " ^ foldl (op ^) "" (map Bool.toString immutables))
         in
  ...
  fun purity purefuncs (Function(rtp, name, tps, formals, reqs, body, ens)) =
    let fun markAndBindArg ((_, t, loc), c) =
        ...
        val ctxAll = foldl (fn (name, c) => mark_struct_fresh c name) ctxArgs (Structtab.list())
        val ctx = buildContext ctxAll body
        val _ = TextIO.output (TextIO.stdErr, "{\n"^AAst.Print.pp_astmt body ^ "\n}\n")
        val _ = TextIO.output (TextIO.stdErr, C.toString ctx ^ "\n")
        fun iter imms =
        ...
        val s = AAst.Print.commas ", " (map Int.toString (IntSet.listItems imms))
        
        val _ = TextIO.output (TextIO.stdErr, "immutables: " ^ s ^ "\n")
