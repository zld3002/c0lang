...
signature PARSE_STATE =
  sig
    ...
    (* mark identifier as typename *)
    val newtype : string -> unit

    (* check whether identifier is a typename *)
...
(* Might want to use a more efficient collection
 * data structure here, but it seems unlikely to
 * be necessary / have a huge ( or even noticable )
 * effect on performance *)
  val currTypes = ref (nil : string list)
  ...
  fun newtype s = currTypes := s :: (!currTypes)

  fun member (s:string) ([]:string list)
      = false
    | member s (e :: L)
      = if s = e then true else member s L

  fun istype s = member s (!currTypes)
end
