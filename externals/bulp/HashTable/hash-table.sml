...
structure MyHashTable : MY_HASH_TABLE =
  struct
  ...
  (* Return list of all keys with the same hash value*)
    fun find_hash (HT{hash_fn, eq_pred, table, ...}) key = let
	  val arr = !table
	  val sz = Array.length arr
	  val hash = hash_fn key
	  val indx = index (hash, sz)
  ...
  (* Return list of all (key,value) with the same hash value*)
    fun find_hashi (HT{hash_fn, eq_pred, table, ...}) key = let
	  val arr = !table
	  val sz = Array.length arr
	  val hash = hash_fn key
	  val indx = index (hash, sz)
