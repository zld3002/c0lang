...
fun chk_oper oper ext =
    ...
    then ( ErrorMsg.error ext
           ("operator '" ^ A.Print.pp_oper oper ^ "' not supported in L2")
...
fun chk_tp (A.Int) ext = ()     (* L1 *)
  ...
  | chk_tp (tp) ext =
    ( ErrorMsg.error ext
      ("type '" ^ A.Print.pp_tp tp ^ "' not supported in L2")
    ; raise ErrorMsg.Error )

fun chk_exp (A.Var _) ext = ()  (* L1 *)
  | chk_exp (A.IntConst _) ext = () (* L1 *)
  | chk_exp (e as A.StringConst _) ext =
    ( ErrorMsg.error ext ("string constants not supported in L2") ; raise ErrorMsg.Error )
  | chk_exp (e as A.CharConst _) ext =
    ( ErrorMsg.error ext ("character constant not supported in L2") ; raise ErrorMsg.Error )
  | chk_exp (e as A.True) ext = () (* L2 *)
  | chk_exp (e as A.False) ext = () (* L2 *)
  | chk_exp (e as A.Null) ext =
    ( ErrorMsg.error ext ("null pointer not supported in L2") ; raise ErrorMsg.Error )
  | chk_exp (e as A.OpExp(A.SUB, _)) ext =
    ( ErrorMsg.error ext ("array access not supported in L2") ; raise ErrorMsg.Error )
  ...
  | chk_exp (A.AllocArray _) ext =
    ( ErrorMsg.error ext ("array allocation not supported in L2") ; raise ErrorMsg.Error )
  | chk_exp (A.Cast _) ext =
    ( ErrorMsg.error ext ("cast not supported in L2") ; raise ErrorMsg.Error )
...
fun chk_stm (A.Assign(NONE, lv, e)) ext = (* L1 *)
  ...
  *)
  | chk_stm (A.Assign(SOME(oper), lv, e)) ext =
    ...
    then ( ErrorMsg.error ext
           ("assignment operator '" ^ A.Print.pp_oper oper ^ "' not supported in L2")
         ; raise ErrorMsg.Error )
  ...
  | chk_stm (A.Continue) ext =
    ( ErrorMsg.error ext ("statement 'continue' not supported in L2") ; raise ErrorMsg.Error )
  | chk_stm (A.Break) ext =
    ( ErrorMsg.error ext ("statement 'break' not supported in L2") ; raise ErrorMsg.Error )
  | chk_stm (A.Return(NONE)) ext =
    ( ErrorMsg.error ext ("return with no value not supported in L2") ; raise ErrorMsg.Error )
  | chk_stm (A.Return(SOME(e))) ext = (* L1 *)
    ( chk_exp e ext )
  | chk_stm (A.Assert(e, _)) ext =
    ( ErrorMsg.error ext ("assertion statements not supported in L2") ; raise ErrorMsg.Error )
  | chk_stm (A.Error _) ext =
    ( ErrorMsg.error ext ("error statements not supported in L2") ; raise ErrorMsg.Error )
  | chk_stm (A.Anno _) ext =
    ( ErrorMsg.error ext ("contract annotations not supported in L2") ; raise ErrorMsg.Error )
...
and chk_specs nil = ()
  | chk_specs (spec::_) =
    ( ErrorMsg.error (spec_ext spec) ("contract annotations not supported in L2") ; raise ErrorMsg.Error )
...
fun chk_gdecl (A.TypeDef(_, _, ext)) =
  ...
  | chk_gdecl (A.Function(g, rtp, params, NONE, specs, _, ext)) =
    ( ErrorMsg.error ext ("function declarations not supported in L2") ; raise ErrorMsg.Error )
  | chk_gdecl (A.Function(g, rtp, params, SOME(s), specs, _, ext)) =
    if Symbol.name g <> "main"
    then ( ErrorMsg.error ext ("only function 'main' may be defined in L2") ; raise ErrorMsg.Error )
    else ( chk_specs specs ; chk_stm s ext )
  | chk_gdecl (A.Pragma(_, ext)) =
    ( ErrorMsg.error ext ("compiler directives not supported in L2") ; raise ErrorMsg.Error )
