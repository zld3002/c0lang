...
fun chk_exp (A.Var _) ext = ()  (* L1 *)
  ...
  | chk_exp (A.AddrOf _) ext =
    ( ErrorMsg.error ext ("address-of operator not supported in L3") ; raise ErrorMsg.Error )
  | chk_exp (A.Invoke _) ext =
    ( ErrorMsg.error ext ("function pointers not supported in L3") ; raise ErrorMsg.Error )
  ...
  | chk_exp e ext = (* impossible? *)
    ( ErrorMsg.error ext ("special identifiers \\result, \\length, \\hastag not supported in L3")
    ; raise ErrorMsg.Error )
...
fun chk_gdecl (A.TypeDef(a, tp, ext)) = (* L3 *)
  ...
  | chk_gdecl (A.FunTypeDef(_, _, _, _, ext)) =
    ( ErrorMsg.error ext ("function type definitions not supported in L3") ; raise ErrorMsg.Error )
