...
fun chk_tp tp ext = case tp of
  ...
  | A.Pointer(A.Void) =>
    ...
    ( ErrorMsg.error ext ("type 'void*' unsupported in language standard 'h0'")
...
fun chk_exp (A.Var _) ext = ()
  ...
  | chk_exp (A.AddrOf(g)) ext =
    ( ErrorMsg.error ext ("function pointers unsupported in language standard 'h0'")
    ; raise ErrorMsg.Error )
  ...
  | chk_exp (A.Cast(tp, e)) ext =
    ( ErrorMsg.error ext ("casting unsupported in language standard 'h0'")
    ; raise ErrorMsg.Error )
...
fun chk_stm (A.Assign(_, e1, e2)) ext =
  ...
  | chk_stm (A.Continue) ext =
    ( ErrorMsg.error ext ("statement 'continue' unsupported in language standard 'h0'")
    ; raise ErrorMsg.Error )
  | chk_stm (A.Break) ext =
    ( ErrorMsg.error ext ("statement 'break' unsupported in language standard 'h0'")
    ; raise ErrorMsg.Error )
...
fun chk_gdecl (A.Struct(s, NONE, _, ext)) = ()
  ...
  | chk_gdecl (A.FunTypeDef(fid, rtp, params, specs, ext)) =
    ( ErrorMsg.error ext ("function type definitions unsupported in language standard 'h0'")
    ; raise ErrorMsg.Error )
  ...
  | chk_gdecl (A.Function(g, rtp, params, SOME(s), specs, _, ext)) =
    ( ErrorMsg.error ext ("function definitions unsupported in language standard 'h0'")
    ; raise ErrorMsg.Error )
