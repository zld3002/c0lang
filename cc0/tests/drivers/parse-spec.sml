        ...
        | go (stack, input as Op oper :: input') =
          let in print "C";
            ...
            | (IsAtm atm2 :: IsOp oper' :: IsAtm atm1 :: stack') =>
              ...
              then go (IsOp oper :: stack, (print "w"; input'))
              else if #prec oper < #prec oper' (* REDUCE *)
              then go (IsAtm (print "y"; #action oper' (atm1, atm2)) :: stack', input)
              else (print "z"; case (#fixity oper, #fixity oper') of
                      (LEFT, LEFT) => (* REDUCE *)
...
structure TestFixity = struct
  ...
  val x0 = resolve "1"
  val x1 = resolve "1 = 9"
  val x2 = resolve "1 + 2 + 3"
  val x3 = resolve "1 & 2 & 3"
  val x4 = resolve "1 * 2 + 3"
  val x5 = resolve "1 + 2 * 3"
  val x6 = resolve "f x + g y"
  val x7 = resolve "f x y + 3"
  val x8 = resolve "3 + f x y"
  val x9 = resolve "3 + f x y + 9"
  val xA = resolve "3 & f x y & 9"
