...
\verb'<sid>', field names \verb'<fid>'.  Variable and function
...
Execution of the \verb'break' statement immediately exits the
innermost loop that contains it and proceeds with the statement
following the loop. The loop invariant is not checked.

Execution of the \verb'continue' statement immediately short-circuits
the body of the innermost loop that contains it and proceeds with the
exit test (in a \verb'while' loop) or the step command followed by the
exit test (in a \verb'for' loop). If contract checking is enabled, 
...
C and declares a \emph{function type name} \verb'<fnid>' which occupies
...
Function types, named by a \verb'<fnid>' are large types and, moreover,
